...
def evaluate(model, valid_loader):
    ...
    for i, data in enumerate(valid_loader, 1):
        ...
        corr = (outputs > 0.5).squeeze().long() == label
        correct += int(corr.sum())
        tot_size += len(label)
    print('val loader size: {}'.format(len(valid_loader.dataset)))
    print('val total size: {}'.format(tot_size))
    return correct/ tot_size
...
def load_model(args):
    ...
    return model, optimizer, loss_fnc
...
def train(args, train_loader, valid_loader):
    ...
    model, optimizer,  loss_fnc = load_model(args)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader, 1):
            ...
            loss = loss_fnc(outputs, label.float())
